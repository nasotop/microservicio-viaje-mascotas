...
public class LoginController {
    ...
    @GetMapping
    public GenericSingleResponseDto<LoginDto> loginUser(@RequestParam("email") String email, @RequestParam("password") String password) {
        GenericSingleResponseDto<LoginDto>  response =  new GenericSingleResponseDto<LoginDto>();

        try{
            response = GenericResponseMapper.ToGenericSingleResponseDto(_loginSvc.LoginWithCredentials(email, password));
        }catch(Exception ex){
